  ...
  <div class="category">
    <div class="search" @click="tosearch">
      <div class="ser">
        ...
        <span>商品搜索,共{{goodsCount}}款好物</span>
      </div>
    ...
    <div class="content">
      ...
      <scroll-view class="right" scroll-y="true">
        ...
        <div class="bottom">
          <div @click="categoryList(item.id)" v-for="(item,index) in detailData.subCategoryList" :key="index" class="item">
...
export default {
  ...
  methods: {
    ...
    async selectitem(id, index) {
      this.nowIndex = index;
      const data = await get("/catalog/current", {
        id: id
      });
      this.detailData = data.data.currentCategory;
      console.log("11111",data)
    },
    async getListData() {
      const data = await get("/catalog/index");
      this.listData = data.data.categoryList;
      console.log("2222",data)
    },
   async  getGoodsCount()
    {
     const data = await post("/goods/count");
